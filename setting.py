...
class Setting:
    ...
    def parse(self):
        ...
        # self.guess_foursquare = True  # foursquare has different default args.
        ...
        # data management
        self.dataset_file = '../flash_back_data/predata/{}'.format(args.dataset)
        self.friend_file = '../flash_back_data/predata/{}'.format(args.friendship)
    ...
    def parse_arguments(self, parser):
        # new
        parser.add_argument('--work_length', default=100, type=int, help='the length to split')
        parser.add_argument('--workpath', default='data', type=str)
        # training
        parser.add_argument('--gpu', default=2, type=int, help='the gpu to use')  # -1
        parser.add_argument('--hidden-dim', default=20 , type=int, help='hidden dimensions to use')
        ...
        # log
        import time
        parser.add_argument('--log_file', default=f'data/{time.asctime(time.localtime(time.time()))}.txt', type=str,
                            help='存储结果日志')  # 修改路径 # '../flash_back_data/predata/日志.txt'
        # parser.add_argument('--trans_loc_file', default='KGE/gowalla_scheme2_transe_loc_temporal_100.pkl', type=str,
        #                     help='loc2loc转换图')
        #coo_loc2loc_distance_normalize_3_0_nowaste.npz
        parser.add_argument('--trans_loc_file', default='data/coo_loc2loc_6_3_nowaste.npz', type=str,
                            help='loc2loc转换图')

        # parser.add_argument('--trans_user_file', default='KGE/bsda_user_similarity_graph.pkl', type=str,
        #                     help='user_sim 图')
    ...
    def parse_gowalla(self, parser):
        ...
        # parser.add_argument('--lambda_s', default=15, type=float, help='decay factor for spatial data')
